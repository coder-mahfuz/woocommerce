...
const { state, actions } = store( 'woocommerce/product-filter-price', {
	...
	actions: {
		getActiveFilterLabel( min: null | number, max: null | number ) {
			const context = getContext< ProductFilterPriceContext >();
			if (
				min &&
				...
				return sprintf(
					context.activeLabelTemplates.minAndMax,
			...
			if ( min && min > context.minRange ) {
				// eslint-disable-next-line @wordpress/valid-sprintf
				return sprintf(
					context.activeLabelTemplates.minOnly,
					formatPrice( min, getCurrency( { minorUnit: 0 } ) )
				);
			}
			...
			if ( max && max < context.maxRange ) {
				// eslint-disable-next-line @wordpress/valid-sprintf
				return sprintf(
					context.activeLabelTemplates.maxOnly,
					formatPrice( max, getCurrency( { minorUnit: 0 } ) )
				);
			}
		...
		setPrice: ( type: 'min' | 'max', value: number ) => {
			...
			productFiltersStore.actions.setActiveFilter( {
				...
				label: actions.getActiveFilterLabel( price.min, price.max ),
				price,
			} );
		},
		...
		clearFilters: () => {
			...
			productFiltersStore.actions.removeActiveFilter( {
				type: 'price',
				value: activeFilterValue( state.minPrice, state.maxPrice ),
			} );
		},
	},
} );
